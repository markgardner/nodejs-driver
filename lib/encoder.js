...
function defineInstanceMembers() {
  ...
  this.encodeList = function (value, subtype, isNestedValue) {
    ...
    for (let i = 0;i < value.length;i++) {
      ...
      const bytes = this.encode(val, subtype, true);
  ...
  this.encodeMap = function (value, subtypes) {
    ...
    function addItem(val, key) {
      ...
      //value
      const valueBuffer = self.encode(val, valueSubtype, true);
      //include item byte length
 ...
 *   </li>
 * </ul>
 * @param {boolean} isNestedValue Whether the value to be encoded is contained within another type (ie. in a list or in a map)
 * @returns {Buffer}
 * @throws {TypeError} When there is an encoding error
 */
Encoder.prototype.encode = function (value, typeInfo, isNestedValue) {
  if (value === undefined) {
  ...
  return encoder.call(this, value, type.info, isNestedValue);
};
